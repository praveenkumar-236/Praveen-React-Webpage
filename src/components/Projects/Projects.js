import React from "react";
import { Container, Row, Col } from "react-bootstrap";
import ProjectCard from "./ProjectCards";
import Particle from "../Particle";
function Projects() {
  return (
    <Container fluid className="project-section">
      <Particle />
      <Container>
        <h1 className="project-heading">
          My Recent <strong className="purple">Works </strong>
        </h1>
        <p style={{ color: "white" }}>
          Here are a few projects I've worked on recently.
        </p>
        <Row style={{ justifyContent: "center", paddingBottom: "10px" }}>
          <Col md={4} className="project-card">
            <ProjectCard 
            imgPath={"https://praveenpkportfolio.netlify.app/static/media/mern-stack.be461fe1.jpg"}
              isBlog={false}
              title="Covid Tracker Website"
              description="Crafted with HTML, CSS, and Javascript, this informative website delivers real-time COVID-19 statistics and news. Featuring a Google Translator, users can access content in their preferred language. The page showcases the latest Ministry of Health tweets and offers additional user-friendly features, enhancing accessibility and engagement. Stay informed effortlessly with this comprehensive COVID-19 hub."
              ghLink="https://github.com/praveenkumar-236?tab=repositories"
              demoLink="#"              
            />
          </Col>
          <Col md={4} className="project-card">
            <ProjectCard
              imgPath={"https://praveenpkportfolio.netlify.app/static/media/react-redux.152d29f5.jpg"}
              isBlog={false}
              title="Student Information Application"
              description="Efficiently manage student information with a dynamic application utilizing Spring Boot for backend operations and ReactJS for frontend. Leveraging MongoDB for data storage, users can seamlessly Add, Delete, Edit, and View student details. This powerful tool streamlines student information management, enhancing administrative processes in educational settings."
              ghLink="https://github.com/praveenkumar-236?tab=repositories"
              demoLink="#"
            />
          </Col>

                    <Col md={4} className="project-card">
            <ProjectCard
              imgPath={"https://praveenpkportfolio.netlify.app/static/media/fullstackjava.0579c4e1.jpg"}
              isBlog={false}
              title="Discount Application Using ODM"
              description="Combining Operational Decision Manager (ODM) and Spring Boot, this innovative app calculates tailored discounts based on selected countries. ReactJS powers the user-friendly frontend, seamlessly interfacing with a MySQL database. By leveraging ODM's capabilities, precise discounts are determined. This integrated solution enhances discount management."
              ghLink="https://github.com/praveenkumar-236?tab=repositories"
              demoLink="#"
            />
          </Col>
          <Col md={4} className="project-card">
            <ProjectCard
              imgPath={"https://praveenpkportfolio.netlify.app/static/media/react.aae10464.png"}
              isBlog={false}
              title="AirLines Ticket Booking"
              description="Crafted with Spring Boot and powered by a Mongo  database, this online airline ticket booking application empowers users to effortlessly secure, view, download, and even cancel their tickets as needed. Catering to frequent flyers, it offers an intuitive interface for seamless self-booking, while providing comprehensive flight information including timings and dates. Designed with user-friendliness in mind, this platform streamlines the ticket booking process, enhancing the travel experience for all."
              ghLink="https://github.com/praveenkumar-236?tab=repositories"
              demoLink="#"
            />
          </Col>
          <Col md={4} className="project-card">
            <ProjectCard
              imgPath={"https://praveenpkportfolio.netlify.app/static/media/html-css-javascript.1d4b020e.jpg"}
              isBlog={false}
              title="Data Security Approach On Cyber Crime With Web Vulnerability"
              description="Developed an advanced predictive tool using Support Vector Machines and Deep Learning techniques, notably LSTM (Long Short-Term Memory), to identify vulnerabilities in web applications. Leveraging the kddcup Dataset, our approach employs Random Forest as a machine learning algorithm. The system efficiently classifies data through progressive layers, enabling effective vulnerability prediction and enhancing web application security."
              ghLink="https://github.com/praveenkumar-236?tab=repositories"
              demoLink="#"
            />
          </Col>
          <Col md={4} className="project-card">
            <ProjectCard
              imgPath={"data:image/jpeg;base64,"}
              isBlog={false}
              title="Portfolio - My Web Page"
              description="Create an immersive developer's portfolio using ReactJS, featuring a homepage showcasing role and education info, project insights, and work experience. Users can seamlessly send emails via the contact page for easy communication. This React-based platform elegantly presents comprehensive developer details, fostering effective self-showcasing."
              ghLink="https://github.com/praveenkumar-236?tab=repositories"
              demoLink="#"
            />
          </Col>
        </Row>
      </Container>
      <h2 style={{color:"white"}}>Next Project<br/>Loading...!</h2>
    </Container>
  );
}

export default Projects;
